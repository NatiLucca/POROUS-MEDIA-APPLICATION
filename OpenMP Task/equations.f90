...
! $omp target map(tofrom:um_tau,vm_tau,p,RU,areau_n, areau_s,areau_e,areau_w,epsilon1,Re,x,y,xm,ym,artDivU,b_art,liga_poros,var)
! $omp teams num_teams(numThreads) thread_limit(32)
    ...
    DO i=3,imax-1
        DO j=2,jmax-1
            res_u(i,j) =( (um(i,j)-um_tau(i,j)) +  RU(i,j)*dt) * 8.d-2 !dtau
...
!$omp parallel do 
    DO i=2,imax-1
	    ...
	    res_v(i,j) =( (vm(i,j)-vm_tau(i,j)) +  RV(i,j)*dt) * 8.d-2 !dtau 
...
subroutine solve_res_v(i,j,m,m_tau,RM,res_m)
    ...
    res_m(i,j) =( (m(i,j)-m_tau(i,j)) +  RM(i,j)*dt) * 8.d-2 !dtau 
...
! $omp target map(to:um_n, areau_e, areau_w, vm_n,areav_n, areav_s) map(from:RP)
! $omp teams num_teams(numThreads) 
!!!!thread_limit(32)
...
!-------contorno esquerdo e direito -------
   ...
   enddo
    ...
    DO i=2,imax-1
        DO j=2,jmax-1
            pi(i,j) = 0.75d0 * p(i,j) + 0.25d0 * (pi(i,j) + 8.d-2 * RP(i,j)* c2)
        ENDDO
    ENDDO
...
!$omp parallel do 
    DO i=2,imax-1
	    ...
	    res_Z(i,j) =( (Z(i,j)- Z_tau(i,j) ) +  RZ(i,j)*dt) * 8.d-2 !!dtau
...
!$omp parallel do 
    DO i=2,imax-1
        DO j=2,jmax-1       
            res_Z(i,j) =( (Z(i,j)-Z_tau(i,j)) +  RZ(i,j)*dt) * 8.d-2
...
subroutine solve_res_Z(i,j,m,m_tau,RM,res_m)
    ...
    res_m(i,j) =( (m(i,j)-m_tau(i,j)) +  RM(i,j)*dt) * 8.d-2
!dtau 
    !res_m(i,j) = RM(i,j) * dt 
