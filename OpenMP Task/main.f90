...
!$omp single 
!$omp target map(tofrom:ui(:,:),vi(:,:),um(:,:),um_n_tau(:,:),vm_n_tau(:,:),um_n(:,:),pi(:,:),um_tau(:,:),res_p(:,:),&
!$omp&rp(:,:),t_tau(:,:),areau_s(:),areau_e(:),areau_n(:),y(:),x(:),vm_tau(:,:),xm(:),epsilon1(:,:),artdivv(:,:),&
!$omp&areav_w(:),vm(:,:),vm_n(:,:),rz(:,:),res_u(:,:),ru(:,:),varz(:),areau_w(:),areav_s(:),areav_e(:),areav_n(:),&
!$omp&artdivu(:,:),liga_poros(:,:),res_z(:,:),p(:,:),pn(:,:),t(:,:),t_n_tau(:,:),var(:),res_v(:,:),rv(:,:),zi(:,:),ym(:), dtau) 
!$omp teams num_teams(numThreads) 

! $omp target update to(dtau, dt,b_art) 
