...
#line 1 "/home/nati/poros_openmp_teams/equations.f90"
subroutine RESU(um,vm,p,RU)

    use comum
    use omp_lib
    implicit none
      include 'equations.f90.opari.inc'
#line 6 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
!!!num_teams(numThreads) thread_limit(32)
  !!!!!!!!!!!!! $omp parallel do private
! $omp parallel PRIVATE(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dudxdx,dxdvdy,q_art,u_W,u_WW,u_E,u_EE,u_N,u_NN,u_S,u_SS,v_P,u_P&
  ! $OMP&,ap,aw,aww,ae,aee,an,ann,as,ass)

  !$omp teams distribute parallel do private(i,j,fn,fs,fe,fw,df,dn,ds,de,dw,dudxdx,dxdvdy,q_art,u_w,u_ww,u_e,u_ee,u_n,u_nn,u_s,u_ss,v_p,u_p&
  !$omp&,ap,aw,aww,ae,aee,an,ann,as,ass)
DO i=3,imax-1
    DO j=3,jmax-2
...
!$omp teams distribute parallel do private(i) num_teams(numthreads) thread_limit(32)
DO i=2,imax
...
!$omp teams distribute parallel do private(j) num_teams(numthreads) thread_limit(32)
DO j=2,jmax-1
...
subroutine upwind_U(um,vm,p,RU,i,j)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 108 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j, itc
...
subroutine solve_U(um,vm,um_n,um_tau,vm_tau,um_n_tau,p,residual_u)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 177 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
!    REAL(8), DIMENSION(1:imax+1,1:jmax+1) ::epsilon1,Darcy
    ...
    !$omp teams distribute parallel do private (i, j) num_teams(numthreads) thread_limit(32)
    DO i=3,imax-1
        DO j=2,jmax-1
...
subroutine solve_res_u(i,j,m,m_tau,RM,res_m)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 260 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
subroutine RESV(um,vm,p,T,InvFr2,RV)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 287 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
!!!!!!! $omp parallel do 
!$omp teams distribute parallel do private(i,j,fn,fs,fe,fw,df,dn,ds,de,dw,dvdydy,dydudx,q_art,&
!$omp&v_w,v_ww,v_e,v_ee,v_n,v_nn,v_s,v_ss,v_p,u_p,ap,aw,aww,ae,aee,an,ann,as,ass) num_teams(numthreads) thread_limit(32)
DO i=3,imax-2
    DO j=3,jmax-1
...
subroutine upwind_V(um,vm,p,RV,InvFr2,T,i,j)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 407 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j, itc
...
subroutine solve_V(um,vm,vm_n,um_tau,vm_tau,vm_n_tau,p,T,InvFr2,residual_v)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 478 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
subroutine solve_res_v(i,j,m,m_tau,RM,res_m)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 563 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
function alpha(x)
    implicit none
      include 'equations.f90.opari.inc'
#line 582 "/home/nati/poros_openmp_teams/equations.f90"
    real(8) :: alpha, x
...
SUBROUTINE quick(ap,aw,aww,ae,aee,an,ann,as,ass,fw,fe,fn,fs,df,Dw,De,Dn,Ds)
   ...
   IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 601 "/home/nati/poros_openmp_teams/equations.f90"
...
SUBROUTINE upwind(ap,aw,ae,an,as,fw,fe,fn,fs,df,Dw,De,Dn,Ds)
   ...
   IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 636 "/home/nati/poros_openmp_teams/equations.f90"
    REAL(8) :: fw, fe, fs, fn, df
...
SUBROUTINE solve_P(c2,p,um_n,vm_n,pn,residual_p)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 661 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
...
SUBROUTINE RESP(um_n,vm_n,p,RP)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 764 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
    REAL(8), DIMENSION(1:imax+1,1:jmax  ) :: um_n
    REAL(8), DIMENSION(1:imax  ,1:jmax+1) :: vm_n
    ...
    !!!!!! $omp parallel do 
    !$omp teams distribute parallel do private(i,j,dudx,dvdy) num_teams(numthreads) thread_limit(32)
    do i=2,imax-1
        do j=2,jmax-1
...
SUBROUTINE solve_Z(um_n,vm_n,Z,Z_n_tau,Z_tau)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 798 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
    REAL(8), DIMENSION(1:imax+1,1:jmax  ) :: um_n
    REAL(8), DIMENSION(1:imax  ,1:jmax+1) :: vm_n
...
!!! RALSTON'S METHOD (Second Order Runge-Kutta)
    ...
    !!! !  $omp parallel do 
    !$omp teams distribute parallel do private(i,j) 
    DO i=2,imax-1
        DO j=2,jmax-1
...
subroutine solve_res_Z(i,j,m,m_tau,RM,res_m)
    ...
    implicit none
      include 'equations.f90.opari.inc'
#line 874 "/home/nati/poros_openmp_teams/equations.f90"
    integer :: i, j
...
SUBROUTINE RESZ(um_n,vm_n,Z,RZ)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 894 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
    REAL(8), DIMENSION(1:imax+1,1:jmax  ) :: um_n
    REAL(8), DIMENSION(1:imax  ,1:jmax+1) :: vm_n
    REAL(8), DIMENSION(1:imax,1:jmax) :: Z, RZ, Zi
 !   REAL(8), DIMENSION(0:imax+1,0:jmax+1) ::epsilon1
    ...
    !!!!$omp parallel do
    !$omp teams distribute parallel do private(i,j, dzudx, dzvdy, de, dw, dn, ds, ze,zw,zn,zs,zp, dp) num_teams(numthreads) thread_limit(32) 
    do i=2,imax-1
        do j=2,jmax-1
...
SUBROUTINE solve_H(H,Z)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 953 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
...
SUBROUTINE comp_T(Z,H,T)
    ...
    IMPLICIT NONE
      include 'equations.f90.opari.inc'
#line 980 "/home/nati/poros_openmp_teams/equations.f90"
    INTEGER :: i, j
