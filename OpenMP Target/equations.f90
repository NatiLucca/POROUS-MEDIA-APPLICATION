...
!    REAL(8), DIMENSION(1:imax+1,1:jmax+1) ::epsilon1,Darcy
  ...
  !$OMP TARGET TEAMS map(tofrom: i,j,areau_n, epsilon1, areau_s, areau_e, areau_w, um(:,:),vm(:,:),ru(:,:), fn,fs,fe,fw,df,Dn,Ds&
!$OMP&,De,Dw,dudxdx,dxdvdy,q_art, artdivU,u_W,u_WW,u_E,u_EE,u_N,u_NN,u_S,u_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass)
...
!   $OMP TEAMS DISTRIBUTE PARALLEL DO private(j) 
DO j=2,jmax-1
...
!    REAL(8), DIMENSION(1:imax+1,1:jmax+1) ::epsilon1,Darcy
    ...
    !$OMP TARGET TEAMS MAP(tofrom: um_tau, res_u, i, j, um, um_tau, RU, res_u, ui) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE (I, J)
    DO i=3,imax-1
        DO j=2,jmax-1
    ...
    !!!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom:I, J, UM, UM_TAU, ru, RES_U, ui, res_u) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i,j) 
    DO i=3,imax-1
        DO j=2,jmax-1
    ...
    !!!!! $omp parallel do
    !$OMP TARGET TEAMS MAP(tofrom: i, j, um,um_tau,RU,res_u, ui) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=3,imax-1
        DO j=2,jmax-1
...
!!!!!!! $omp parallel do 
!$OMP TARGET TEAMS MAP(tofrom:i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,&
!$omp&v_W,v_WW,v_E,v_EE,v_N,v_NN,v_S,v_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass)
!$OMP DISTRIBUTE PARALLEL DO private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,&
!$omp&v_W,v_WW,v_E,v_EE,v_N,v_NN,v_S,v_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass)
DO i=3,imax-2
    DO j=3,jmax-1
...
!    REAL(8), DIMENSION(1:imax+1  ,1:jmax+1) :: epsilon1,Darcy
    ...
    !!!!!  $omp parallel do private(i,j) 
    !$OMP TARGET TEAMS MAP(tofrom: i,j,vm,vm_tau,RV,res_v, vi) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i, j) 
    DO i=2,imax-1
        DO j=3,jmax-1
    ...
    !!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom:i,j,vm,vm_tau,RV,res_v, vm_n_tau, vi) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=3,jmax-1
            call solve_res_v(i,j,vm,vm_tau,RV,res_v)
...
!mass conservation
    ...
    !!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom: pi, p, dtau, RP, c2) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=2,jmax-1
...
!!!!!$omp end parallel do
    ...
    !!!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom:res_p, dtau, RP, c2, pn, p, pi, res_p) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=2,jmax-1
...
SUBROUTINE RESP(um_n,vm_n,p,RP)
    ...
    !!!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom: dudx, um_n, areau_e, um_n, areau_w, dvdy, vm_n, areav_n, areav_s, vm_n, RP) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j,dudx,dvdy) 
    do i=2,imax-1
        do j=2,jmax-1
...
!!! RALSTON'S METHOD (Second Order Runge-Kutta)
    ...
    !!! !  $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom: i,j,Z,Z_tau,RZ,res_Z, zi)
    !$OMP  DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=2,jmax-1
...
!!!!! $omp parallel do private(i,j)
    !$OMP TARGET TEAMS MAP(tofrom: i,j,Z,Z_tau,RZ,res_Z, Z_N_TAU, zi) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i, j)  
    DO i=2,imax-1
        DO j=2,jmax-1
...
SUBROUTINE RESZ(um_n,vm_n,Z,RZ)
 ...
 !   REAL(8), DIMENSION(0:imax+1,0:jmax+1) ::epsilon1
    ...
    !!!!$omp parallel do
    !$OMP TARGET TEAMS MAP(tofrom: dZudx, dZvdy, De, Dw, Dn, Ds, Ze,Zw,Zn,Zs,Zp, Dp) MAP(to:Z, um_n, areau_e, areau_w, areav_n, areav_s&
!$OMP&, ym, xm, Pe, x, y) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j, dZudx, dZvdy, De, Dw, Dn, Ds, Ze,Zw,Zn,Zs,Zp, Dp) 
    do i=2,imax-1
        do j=2,jmax-1
...
SUBROUTINE comp_T(Z,H,T)
    ...
    !!!!!#omp parallel do 
!$OMP TARGET TEAMS MAP(tofrom: T, q, H, Z, S) 
!$OMP DISTRIBUTE PARALLEL DO private(i,j)
    do i=1,imax
        do j=1,jmax
